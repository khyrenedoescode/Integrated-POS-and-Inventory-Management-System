...
namespace POSales
{
    public partial class DailySale : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            if (cboCashier.Text == "All Cashier")
            {
                query = "SELECT c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc as discount, c.total " +
                        "FROM tbCart AS c INNER JOIN tbProduct AS p ON c.pcode = p.pcode " +
                        "WHERE status LIKE 'Sold' AND sdate BETWEEN @FromDate AND @ToDate";
            }
            else
            {
                query = "SELECT c.id, c.transno, c.pcode, p.pdesc, c.price, c.qty, c.disc as discount, c.total " +
                        "FROM tbCart AS c INNER JOIN tbProduct AS p ON c.pcode = p.pcode " +
                        "WHERE status LIKE 'Sold' AND sdate BETWEEN @FromDate AND @ToDate AND cashier LIKE @Cashier";
            }

            using (SqlConnection connection = new SqlConnection("Data Source=LAPTOP-V7CATOCU\\SQLEXPRESS;Initial Catalog=MdemyPOSFINAL;Integrated Security=True"))
            {
                connection.Open();

                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@FromDate", dtFrom.Value.ToString(dateFormat));
                    command.Parameters.AddWithValue("@ToDate", dtTo.Value.ToString(dateFormat));
                    command.Parameters.AddWithValue("@Cashier", cboCashier.Text);

                    using (SqlDataAdapter adapter = new SqlDataAdapter(command))
                    {
                        DataTable dataTable = new DataTable();
                        adapter.Fill(dataTable);
                        report.LoadDailyReport(dataTable, param, cboCashier.Text);
                    }
                }
            }
